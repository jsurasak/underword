...
$(function() {
          ...
          order:[ 0 ,'desc'],
          ajax: {
    			url: '<?php echo e(action("WritherManageNovelController@datatable")); ?>',
    			data: function ( d ) {},
                method: 'POST'
            },
          columns: [
          ...
          rowCallback: function(nRow, aData, dataIndex){
            ...
            $('td:eq(2)',nRow).html(''
                +'<img src ="<?php echo e(url("../")); ?>/'+aData['picture']+'" width="100px">'                    
            );
...
function Check(type){
  ...
  if(count > 5){
    ...
    oTable.draw();
     ...
     $.ajax('<?php echo e(action("WritherManageNovelController@top_nove")); ?>', {
         type: 'POST',
         data:myform,
         dataType: 'json',
...
function editR(d,e){
    ...
    $.ajax('<?php echo e(action("WritherManageNovelController@updaterating")); ?>', {
        type: 'POST',
        data: {
            'id':id,
            'rating':data
...
function delNovel(id){
    ...
    if(confirm("ยืนยันการลบหนังสือ")){
        ...
        $.ajax('<?php echo e(action("WritherManageNovelController@delnovel")); ?>', {
        type: 'POST',
        data: {
            'id':id,
        },
