<?php $__env->startSection('content'); ?>
		<div id="content" class="content">
			...
			<ol class="breadcrumb pull-right">
				...
				<li class="active">เรื่องเล่า</li>
			</ol>
			<!-- end breadcrumb -->
			<!-- begin page-header -->
			<h1 class="page-header">จัดการ เรื่องเล่า</h1>
			<!-- end page-header -->
			    ...
			    <div class="col-md-12">
                    ...
                    <div class="panel panel-inverse">
                        <div class="panel-heading">
                            ...
                            <h4 class="panel-title">จัดการ เรื่องเล่า/ประสบการณ์</h4>
                        </div>
...
$(function() {
          ...
          order:[ 0 ,'desc'],
          ajax: {
    			url: '<?php echo e(action("WritherManageOtherController@datatable_story")); ?>',
                method: 'POST'
            },
          columns: [
          ...
          rowCallback: function(nRow, aData, dataIndex){
            ...
            $('td:last-child',nRow).html(''
                +'<a href="manage_story/'+aData['id']+'/form"  class="btn btn-xs btn-success">แก้ไข</a>'
            ...
            if(aData['status'] == 0){
                $('td:eq(3)',nRow).html('<p>ยังไม่เปิดให้อ่าน</p>');
            }else if(aData['status'] == 2){
                $('td:eq(3)',nRow).html('<p>บันทึกร่าง</p>');
            }else if(aData['status'] == 1){
                $('td:eq(3)',nRow).html('<p>เปิดให้อ่าน</p>');
...
function Check(type){
  ...
  if(count > 6){
    swal({
        title: "Warning!!!",
        text: "ไม่สามารถเลือกได้เกิน 6 เรื่อง",
        icon: "warning",
      });
    oTable.draw();
  }else{

     $.ajax('<?php echo e(action("WritherManageOtherController@top_Story")); ?>', {
         type: 'POST',
         data:myform,
         dataType: 'json',
...
function review(id){
	...
	$('#review_iframe').html("<iframe src='<?php echo e(url('../')); ?>/review-story/"+id+"/' width='100%' height='100%'></ifram>");
...
if (value != null) {
   window.location.href="manage_story/"+id+"/close/"+value;
}
...
function delNovel(id){
    ...
    if(confirm("ยืนยันที่จะลบข้อมูล จะไม่สามารถนำกลับมาได้อีก")){
        window.location.href="manage_story/"+id+"/del";
    }
...
function editV(d,e){
    ...
    var id = $(d).data('id');
     ...
     $.ajax('<?php echo e(action("WritherManageOtherController@updateview")); ?>', {
        type: 'POST',
        data: {
            'id':id,
            'view':data
