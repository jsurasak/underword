		  ...
		  <div class="modal-dialog modal-lg" >
		    ...
		    <div class="modal-content">
		      ...
		      <div class="modal-body" style="height: 100%;">
                <form id="myform" method="POST" action="{{ action('DefaultpicController@update')}}" enctype="multipart/form-data">
                {{ csrf_field() }}
...
$(function() {
          ...
          serverSide: true,
          ajax: {
    			url: '{{ action("SkullManageController@datatable") }}',
    			data: function ( d ) {},
                method: 'POST'
            },
          columns: [
          ...
          rowCallback: function(nRow, aData, dataIndex){
             ...
             $('td:eq(0)',nRow).html('<p onclick="editD(this);" data-id="'+aData['id']+'" >'+aData['detail']+' <img src="{{ url("../") }}/img/icon/skull.png" width="25" ></p>');
...
function editD(d){
    ...
    var detail = prompt('ใส่จำนวนที่ต้องการแก้ไข');
        ...
        if (detail != null) {
        $.ajax('{{ action("SkullManageController@updateD") }}', {
            type: 'POST',
            data: {
                id:id,
                detail:detail
...
function editP(d){
    ...
    var detail = prompt('ใส่จำนวนที่ต้องการแก้ไข');
        ...
        if (detail != null) {
        $.ajax('{{ action("SkullManageController@updateP") }}', {
            type: 'POST',
            data: {
                id:id,
                detail:detail
