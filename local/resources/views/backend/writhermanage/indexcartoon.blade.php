			    ...
			    <div class="col-md-12">
                    ...
                    <div class="panel panel-inverse">
                        <div class="panel-heading">
                            ...
                            <h4 class="panel-title">จัดการ {{ $book->name }}</h4>
                        </div>
...
$(function() {
          ...
          serverSide: true,
          ajax: {
    			url: '{{ action("WritherManageCartoonController@datatable_book") }}',
    			data: function ( d ) { 
                    d.id = $('#id').val();
                },
                method: 'POST'
          ...
          rowCallback: function(nRow, aData, dataIndex){
            ...
            if(aData['chapter'] == 0){
             ...
             $('td:last-child',nRow).html(''
                +'<a onclick="closeNovel('+aData['id_novel']+','+aData['id']+');" class="btn btn-warning" >ปิดตอน</a>'
            );
            ...
            if(aData['status'] == 0){
               ...
               $('td:eq(4)',nRow).html('<p>ถูกลบโดย User</p>'); 
            }
...
function review(data){
	...
	$('#review_iframe').html("<iframe src='{{ url('../') }}/"+id+"/comic-review/"+chap+"/' width='100%' height='100%'></ifram>");
...
if (value != null) {
   window.location.href="manage_comics/"+id+"/book_novel/"+chap+"/close/"+value;
}
...
function editV(d,e){
    ...
    var id = $(d).data('id');
     ...
     $.ajax('{{ action("WritherManageCartoonController@updateview") }}', {
        type: 'POST',
        data: {
            'id':id,
            'view':data
...
function delBook(id){
    ...
    if(confirm("ยืนยันการลบหนังสือ")){
        ...
        $.ajax('{{ action("WritherManageCartoonController@delBook") }}', {
        type: 'POST',
        data: {
            'id':id,
        },
