...
@section('content')
		<div id="content" class="content">
			...
			<ol class="breadcrumb pull-right">
				...
				<li class="active">การ์ตูน</li>
			</ol>
			<!-- end breadcrumb -->
			<!-- begin page-header -->
			<h1 class="page-header">จัดการ การ์ตูน</h1>
			<!-- end page-header -->
			    ...
			    <div class="col-md-12">
                    ...
                    <div class="panel panel-inverse">
                        <div class="panel-heading">
                            ...
                            <h4 class="panel-title">จัดการ การ์ตูน</h4>
                        </div>
...
$(function() {
          ...
          order:[ 0 ,'desc'],
          ajax: {
    			url: '{{ action("WritherManageCartoonController@datatable") }}',
    			data: function ( d ) {},
                method: 'POST'
            },
          columns: [
            {title :'แนะนำ',data:'TopSelect', className: 'text-center', defaultContent: '-'},
          ...
          rowCallback: function(nRow, aData, dataIndex){
            ...
            $('td:eq(0)',nRow).html('<input type="checkbox" name="topCom[]" value="'+aData['id']+'" onclick="Check(this);" >');
             ...
             $('td:last-child',nRow).html(''
                ...
                +'<a href="manage_comics/'+aData['id']+'/book_comics"  class="btn btn-sm btn-success">ดูบทข้างใน</a> '
                +'<a onclick="closeNovel('+aData['id']+');" class="btn btn-warning" >ปิดตอน</a>'                    
            );
...
function closeNovel(id){
    ...
    if (value != null) {
    window.location.href="manage_comics/"+id+"/close/"+value;
    }
...
function editR(d,e){
    ...
    var id = $(d).data('id');
     ...
     $.ajax('{{ action("WritherManageCartoonController@updaterating") }}', {
        type: 'POST',
        data: {
            'id':id,
            'rating':data
...
function Check(type){
  ...
  if(count > 10){
    swal({
        title: "Warning!!!",
        text: "ไม่สามารถเลือกได้เกิน 10 เรื่อง",
        icon: "warning",
      });
    oTable.draw();
  }else{

     $.ajax('{{ action("WritherManageCartoonController@top_nove") }}', {
         type: 'POST',
         data:myform,
         dataType: 'json',
         success: function(data) {
...
function delComics(id){


    if(confirm("ยืนยันการลบหนังสือ")){

        $.ajax('{{ action("WritherManageCartoonController@delComics") }}', {
        type: 'POST',
        data: {
            'id':id,
        },
