...
$(function() {
          ...
          order:[ 0 ,'desc'],
          ajax: {
    			url: '{{ action("WritherManageOtherController@datatable_story") }}',
                method: 'POST'
            },
          columns: [
...
function Check(type){
  ...
  if(count > 6){
    ...
    oTable.draw();
     ...
     $.ajax('{{ action("WritherManageOtherController@top_Story") }}', {
         type: 'POST',
         data:myform,
         dataType: 'json',
...
function review(id){
	...
	$('#review_iframe').html("<iframe src='{{ url('../') }}/review-story/"+id+"/' width='100%' height='100%'></ifram>");
