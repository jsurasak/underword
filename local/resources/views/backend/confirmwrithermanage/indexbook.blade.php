			    ...
			    <div class="col-md-12">
                    ...
                    <div class="panel panel-inverse">
                        <div class="panel-heading">
                            ...
                            <h4 class="panel-title">จัดการตอนนิยายที่ถูกแก้ไข</h4>
                        </div>
...
$(function() {
          ...
          serverSide: true,
          ajax: {
    			url: '{{ action("ConfirmWritherManageOtherController@datatable_book") }}',
                method: 'POST'
            },
          columns: [
          ...
          rowCallback: function(nRow, aData, dataIndex){
            ...
            if(aData['chapter'] == 0){
             ...
             $('td:last-child',nRow).html(''
                +'<a onclick="closeNovel('+aData['id']+');" class="btn btn-sm btn-warning" >ไม่อนุมัติ</a>'
                +'<a onclick="confirmNovel('+aData['id']+');" class="btn btn-sm btn-success" >อนุมัติ</a>'
            );
...
function review(data){
	...
	$('#review_iframe').html("<iframe src='{{ url('../') }}/"+id+"/novel-review/"+chap+"/' width='100%' height='100%'></ifram>");
...
var value = prompt("กรุณา กรอกสาเหตุที่ปิดตอน");
    ...
    if (value != null) {
    window.location.href="confirm_chaper_novel/"+id+"/close/"+value;
    }

}

function confirmNovel(id){

    if(confirm("ยืนยัน การอนุมัติ")){

        window.location.href="confirm_chaper_novel/"+id+"/confirm";
