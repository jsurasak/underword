...
$(function() {
          ...
          order:[ 0 ,'desc'],
          ajax: {
    			url: '{{ action("ConfirmWritherManageOtherController@datatable_story") }}',
                method: 'POST'
            },
          columns: [
          ...
          rowCallback: function(nRow, aData, dataIndex){
            ...
            $('td:last-child',nRow).html(''
                +'<a onclick="closeStory('+aData['id']+');" class="btn btn-sm btn-warning" >ปิดตอน</a>'
                +'<a onclick="confirmStory('+aData['id']+');" class="btn btn-sm btn-success" >อนุมัติ</a>'
            );
...
if (value != null) {
   window.location.href="confirm_story/"+id+"/close/"+value;
}

}

function confirmStory(id){

    if(confirm("ยืนยัน การอนุมัติ")){

        window.location.href="confirm_story/"+id+"/confirm";
