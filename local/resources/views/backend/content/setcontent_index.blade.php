...
$(function() {
          ...
          serverSide: true,
          ajax: {
    			url: '{{ action("SetcontentController@datatableTopView") }}',
    			data: function ( d ) {},
                method: 'POST'
            },
          columns: [
    ...
    oTablec = $('#data-table_Top5c').DataTable({
        ...
        serverSide: true,
        ajax: {
        url: '{{ action("SetcontentController@datatableTop5Content") }}',
        data: function ( d ) {},
              method: 'POST'
          },
        columns: [
...
function topstatus(sT,id){
    ...
    var status = $(sT).data('status');
      ...
      $.ajax('{{ action("SetcontentController@UpdateStatus") }}', {
          type: 'POST',
...
function Check(type){
  ...
  if(count > 5){
    ...
    $.ajax('{{ action("SetcontentController@UpdateCheckStatus") }}', {
        type: 'POST',
        data:myform,
        dataType: 'json',
        success: function(data) {
